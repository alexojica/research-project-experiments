...
class CTGANSynthesizer(LossValuesMixin, BaseSingleTableSynthesizer):
    ...
    def __init__(self, metadata, processed_data, enforce_min_max_values=True, enforce_rounding=True,
        ...
        self._model = CTGAN(processed_data, discrete_columns, **self._model_kwargs)
    ...
    def set_weights(self, weights):
        ...
        self._model._generator.load_state_dict(weights['generator'])
        self._model._discriminator.load_state_dict(weights['discriminator'])

    def get_weights(self):
        return {'generator': self._model._generator.state_dict(),
                'discriminator': self._model._discriminator.state_dict()}
...
class TVAESynthesizer(LossValuesMixin, BaseSingleTableSynthesizer):
    ...
    def __init__(self, metadata, processed_data, enforce_min_max_values=True, enforce_rounding=True,
        ...
        transformers = self._data_processor._hyper_transformer.field_transformers
        self.discrete_columns = detect_discrete_columns(
            self.get_metadata(),
            processed_data,
            transformers
        )
        self._model = TVAE(processed_data, self.discrete_columns, **self._model_kwargs)

    def set_weights(self, weights):
        """Set the weights of the model."""
        self._model.encoder.load_state_dict(weights['encoder'])
        self._model.decoder.load_state_dict(weights['decoder'])

    def get_weights(self):
        return {'encoder': self._model.encoder.state_dict(),
    ...
    def _fit(self, processed_data):
        ...
        Args:
            processed_data (pandas.DataFrame):
                Data to be learned.
        """
        self._model.fit(processed_data, discrete_columns=self.discrete_columns)
